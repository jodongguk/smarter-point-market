<script lang="ts">
	...
	function onClickCouponModal(om: OrderMaster) {
		orderMaster = om;
...
<div class="flex flex-col p-8 border rounded bg-white">
	...
	<div class="mt-4 flex justify-between space-x-16">
		<div class="flex flex-col flex-1">
			...
			<DataColumn title="주문일시">
				<MdBlack>
					{formatMilitaryTime(orderMaster.dateCreated)}
				</MdBlack>
			</DataColumn>
		...
		<div class="flex flex-col flex-1">
			...
			<DataColumn title="결제확인일시">
				<MdBlack>
					{orderMaster.datePaid ? formatMilitaryTime(orderMaster.datePaid) : '-'}
				</MdBlack>
			</DataColumn>
		</div>
...
{#each orderMaster.additionalOrders as additionalOrder}
	<div class="flex flex-col p-8 border rounded bg-white">
		...
		<div class="flex flex-col my-4">
			...
			>
			<DataColumn title="배송비">{formatMoney({ money: additionalOrder.priceDelivery })}</DataColumn
			>
			<DataColumn title="결제할 금액"
