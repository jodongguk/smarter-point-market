<script lang="ts">
    ...
    if(get(userSelectChildren)) {
        userSelectChildren.set(data.userMe?.children[0])
    }
    ...
    <svelte:fragment slot="header">
        <AppBar gridColumns="grid-cols-3" slotDefault="place-self-center" slotTrail="place-content-end">
            <svelte:fragment slot="lead">
                <button type="button" on:click={mypageByParent}>
        ...
        <div class="flex">
            ...
            <button class="flex-1 py-2 font-semibold {checkMenuActiveClass('badge')}" on:click="{ () => goto('badge') }">칭찬스티커</button>
            <button class="flex-1 py-2 font-semibold {checkMenuActiveClass('pointland')}"on:click="{ () => goto('pointland') }">포인트랜드</button>
        </div>
